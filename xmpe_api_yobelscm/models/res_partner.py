...
class Partner(models.Model):
    ...
    yobel_sync = fields.Boolean(
        string=_('Yobel Synchronization'), default=False
    )
    ...
    origin_date = fields.Datetime(
        string=_('Origin Date'), default=fields.Datetime.now
    )
            ...
            shipment_list.append(
                {
                    ...
                    'CLIFPR': self.origin_date.date().strftime(
                        DEFAULT_SERVER_DATE_FORMAT
                    ),
                    'CLICCL': rec.vat,
                    'CLINBR': rec.name,
