...
url_yobel = {
    'order_test':
    'http://yscmserver-test.yobelscm.biz:1973/TI_Logistics'
    '/WSYOB_RECEP_LOG/WSYOB_RECEP/CrearPedido',
    'order_prod':
    'http://yscmserver-04.yobelscm.biz:1973/WSYOB_RECEP_LOG'
    '/WSYOB_RECEP/CrearPedido',
    'shipment_test':
    'http://yscmserver-test.yobelscm.biz:1973/TI_Logistics'
    '/WSYOB_RECEP_LOG/WSYOB_RECEP/CrearEmbarque',
    'shipment_prod':
    'http://yscmserver-04.yobelscm.biz:1973/WSYOB_RECEP_LOG/WSYOB_RECEP/CrearEmbarque'
}
order_test = {
    "Seguridad": {
        "compania": "LIB",
...
class Picking(models.Model):
    ...
    origin_date = fields.Datetime(
        string=_('Origin Date'),
        default=fields.Datetime.now,
    )
    yobel_state = fields.Selection(string=_('Yobel State'),
                                   selection=YOBEL_STATE,
                                   default='',
                                   readonly=True)

    id_mensaje_out = fields.Char(string=_('Mensaje ID'),
                                 copy=False,
                                 readonly=True,
                                 index=True,
                                 default=lambda self: _('New'))

    id_mensaje_in = fields.Char(string=_('Mensaje ID'),
                                copy=False,
                                readonly=True,
                                index=True,
                                default=lambda self: _('New'))
    ...
    def fill_message(self):
        ...
        for rec in self:
            ...
            for detail in rec.move_ids_without_package:
                detail_list.append({
                    ...
                    detail.id,
                    "EMBPRO":
                    detail.product_id.default_code[:30],
                    "EMBQTY":
                    int(detail.product_uom_qty),
                    "EMBUMC":
                    detail.product_uom.name[:10],
                    "EMBLOT":
                    detail.product_id.name[:25],
                    "EMBFVE":
            ...
            shipment_list.append({
                "EMBCIA":
                self.env.company.yobel_identifier,
                "EMBNRO":
                rec.name,
                "EMBFA1":
                rec.scheduled_date.strftime(DEFAULT_SERVER_DATE_FORMAT),
                "EMBOCP":
                rec.name[:6],
                "EMBPRV1":
                rec.company_id.vat or '2021231214',
                "EMBPOR":
                rec.company_id.state_id.code,
                "EMBNCT":
                "0001",
                "EMBA01":
                "",
        ...
        return {
            "Head": {
                ...
                "SAP",
                "fecha_origen":
                fields.Datetime.now().strftime(DEFAULT_SERVER_DATE_FORMAT),
                "tipo":
                "RECEMB"
            },
            "Body": {
    ...
    def send_yobel_shipment_data(self):
        if self.yobel_sync:
            if self.picking_type_code != 'outgoing':
                ...
                self.write({
                    'id_mensaje_in':
                    self.env['ir.sequence'].next_by_code('yobel_master_emb')
                })
                data = {
                    "Seguridad": self.fill_security(),
                    "Mensaje": self.fill_message()
                }
                is_api_test = ICPSudo.get_param(
                    'xmpe_api_yobelscm.is_api_test')
                if is_api_test:
    ...
    def fill_order_message(self):
        ...
        for rec in self:
            ...
            for detail in rec.move_ids_without_package:
                detail_list.append({
                    ...
                    'lote',
                    "PEDCTD":
                    int(detail.quantity_done),
                    "PEDALM":
                    detail.product_id.warehouse_id.name or '',
                    "PEDSKU":
                    detail.product_id.supplier_sku_code,
                    "PEDUXE":
                    detail.max_unit_per_package_picking or '',
                    "PEDA02":
                    detail.additional_2 or '',
                    "PEDN02":
                    detail.numeric_2,
                    "PEDF02":
                    detail.flag_2 or '',
            ...
            order_list.append({
                ...
                rec.name,
                "PEDFCH":
                rec.origin_date.strftime(DEFAULT_SERVER_DATE_FORMAT),
                "PEDTIT":
                len(detail_list),
                "PEDTUN":
                sum([d['PEDCTD'] for d in detail_list]),
                "PEDFEI":
                rec.scheduled_date.strftime(
                ...
                rec.scheduled_date.strftime(
                    DEFAULT_SERVER_DATETIME_FORMAT)[:10],
                "PEDHOF":
                rec.scheduled_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
                [11:],
                "PEDNOC":
                '',
                "PEDTDA":
        ...
        return {
            "Head": {
                ...
                "SAP",
                "fecha_origen":
                self.origin_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                "tipo":
                "RECPED"
            },
            "Body": {
        ...
        if self.yobel_sync:
            if self.picking_type_code == 'outgoing':
                ...
                if is_api_test:
                    ...
                    req = self.send_yobel_data(url_yobel['order_test'],
                                               order_test)
                else:
