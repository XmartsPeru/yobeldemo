...
class Picking(models.Model):
    ...
    yobel_sync = fields.Boolean(string=_('Yobel Synchronization'), default=False)
    sync = fields.Boolean(string=_('Synchronization'), default=False)
    notify_message = fields.Char(
        string=_('Notify Message'),
    )
    ...
    additional_1 = fields.Char(
        string=_('Additional 1'),
    )
    numeric_1 = fields.Integer(
        string=_('Numeric 1'),
    )
    ...
    yobel_state = fields.Selection(
        string=_('Yobel State'),
        selection=YOBEL_STATE,
        default='',
        readonly=True,
    )

    id_mensaje_out = fields.Char(
        string=_('Mensaje ID'),
        copy=False,
        readonly=True,
        index=True,
    ...
    def fill_message(self):
        ...
        return {
            'Head': {
                ...
                'sistema_origen': 'SAP',
                'fecha_origen': fields.Datetime.now().strftime(
                    DEFAULT_SERVER_DATE_FORMAT
                ),
                'tipo': 'RECEMB',
            },
            'Body': {'Embarques': shipment_list},
        }
        ...
        if self.yobel_sync:
            if self.picking_type_code != 'outgoing':
                ...
                if is_api_test:
                    ...
                    req = self.send_yobel_data(url_yobel['shipment_test'], data_test)
                else:
    ...
    def fill_order_message(self):
            ...
            order_list.append(
                {
                    ...
                    'PEDFEI': rec.scheduled_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)[
                        :10
                    ],
                    'PEDHOI': rec.scheduled_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)[
                        11:
                    ],
                    'PEDFEF': rec.scheduled_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)[
                        :10
                    ],
                    'PEDHOF': rec.scheduled_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT)[
                        11:
                    ],
                    'PEDNOC': '',
                    'PEDTDA': rec.store or '',
        ...
        return {
            'Head': {
                ...
                'sistema_origen': 'SAP',
                'fecha_origen': self.origin_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT),
                'tipo': 'RECPED',
            },
            'Body': {'Pedidos': order_list},
        }
...
class StockMove(models.Model):
    ...
    additional_2 = fields.Char(
        string=_('Additional 2'),
    )
    numeric_2 = fields.Integer(
        string=_('Numeric 2'),
    )
    # Only OUT
    flag_2 = fields.Integer(
        string=_('Flag 2'),
    )
